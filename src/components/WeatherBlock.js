  ...
  getDarkSkyAPI = date => {
        ...
        axios.get(`${CORS}${API_URL}${API_KEY}/${cognizantLatLong}${date}${excludeBlocks}`)
            .then(response => { 
              ...
              if (forecastDay) {
                const summary = dsData.daily.data[forecastDay].summary;
                const skyIcon = dsData.daily.data[forecastDay].icon;
                const temperature = Math.round(dsData.daily.data[forecastDay].temperatureHigh);
                const tempHigh = Math.round(dsData.daily.data[forecastDay].temperatureHigh);
                const tempLow = Math.round(dsData.daily.data[forecastDay].temperatureLow);
                const time = this.toTextTime(dsData.daily.data[forecastDay].time);

                this.setState({ skyIcon, summary, temperature, tempHigh, tempLow, doneLoading: true, timeMachineDate: time });
